...
// 	// Add the current request to the tracking map
// 	rl.requests[ip] = append(rl.requests[ip], now)
// 	requestsFromThisIp := len(rl.requests[ip])

// 	// Check for the "Lambda" pattern
// 	if uniqueIps > rl.multiIpBurst {
// 		log.Printf("[THROTTLING] Detected distributed load from %d unique IPs. Simulating UNCOOPERATIVE HANG for %s.", uniqueIps, ip)
// 		// We can unlock here because hijacking takes over the connection
// 		rl.mutex.Unlock()
// 		hijackAndHang(w, r)
// 		return
// 	}

// 	// Check for the "Local Test" pattern
// 	if requestsFromThisIp > rl.singleIpBurst {
// 		delay := time.Duration(requestsFromThisIp-rl.singleIpBurst) * 4 * time.Second
// 		log.Printf("[QUEUEING] Detected single-source burst from %s (%d requests). Delaying response by %s.", ip, requestsFromThisIp, delay)

// 		// =================================================================
// 		// THE FIX: The time.Sleep() now happens INSIDE the mutex lock.
// 		// This forces each request to wait for the previous one to finish,
// 		// creating a real queue.
// 		// =================================================================
// 		time.Sleep(delay)
// 		rl.mutex.Unlock() // Unlock AFTER sleeping
...
// 	rl.mutex.Unlock() // Unlock for normal requests
// 	log.Printf("[OK] Normal request from %s. Responding immediately.", ip)
// 	w.WriteHeader(http.StatusAccepted)
// 	w.Write([]byte("202 Accepted"))
// }

// func hijackAndHang(w http.ResponseWriter, r *http.Request) {
...
// 	log.Println("Starting intelligent rate-limiting server on http://localhost:8083...")
